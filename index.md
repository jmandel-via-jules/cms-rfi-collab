...
All authorized users and their designated tools and applications must have widespread, timely, and efficient access to both standardized data (e.g., USCDI via individual and bulk FHIR APIs) and to complete, computable Electronic Health Information (EHI via API) as an essential underlying access point. Usability and performance of these access methods are paramount.
...
5.  **Facilitation of Individual Data Retrieval within this Architecture:** Within this trustworthy and flexible framework, QHINs must provide or ensure individuals have access to functionalities enabling them to:
    ...
    *   Initiate cost-free queries for their *own* USCDI data (and eventually their full Electronic Health Information, **REVIEW NEEDED: The concept `req_api_ehi_as_tefca_source` is referenced here but not defined in this document.**) from all participating data holders via TEFCA.
...
*   **Comprehensive National Provider & Health IT Endpoint Directory:**
    ...
    *   **Data Quality & Maintenance:** Clear processes for data submission, validation, and regular updates must be established to ensure accuracy and timeliness, potentially leveraging existing data sources (e.g., NPPES - National Plan and Provider Enumeration System) but enhancing them with necessary interoperability-specific details.
